...
public class WardrobeIconLoader implements com.sky.modelviewer.ui.WardrobeGridAdapter.IconLoader {
    ...
    public Bitmap loadIcon(String entryName) {
        ...
        if (info == null || info.iconName == null || info.iconName.isEmpty()) return null;
        ...
        if (region == null) {
            ...
            // Some icons (seasonal pendants etc.) are not in UIPackedAtlas but exist as independent KTX files
        ...
        if (atlas == null) {
            // Atlas decode failed — also try standalone KTX as fallback
            Bitmap bmp = loadStandaloneKtxIcon(entryName, info);
            bitmapCache.put(entryName, bmp);
            return bmp;
        }
    ...
    private Etc2Decoder.DecodedImage ensureAtlasDecoded(String atlasName) {
        ...
        try {
            // texIndex keys are lowercase base names (no path, no extension).
            // HTML: texIndex.get(key) — direct lookup
            String entryPath = texIndex.get(key);
            if (entryPath == null) {
            ...
            // HTML: decodeR11Ktx(raw) || decodeKtx(raw)
            // Etc2Decoder.decodeKtx handles both R11_EAC and ETC2 formats
