...
public class UIAtlasParser {
    ...
    public static HashMap<String, ImageRegion> parseAtlas(String apkPath) {
        ...
        try {
            ...
            // 尝试加载网易版UIPackedAtlasNetease
            try {
                String neteasePath = SkyAssetScanner.findResourceEntry(apkPath, "UIPackedAtlasNetease.lua");
                if (neteasePath != null) {
                    Log.i(TAG, "Found UIPackedAtlasNetease at: " + neteasePath);
                    byte[] neteaseBytes = SkyAssetScanner.readApkEntry(apkPath, neteasePath);
                    if (neteaseBytes != null) {
                        int before = regions.size();
                        if (LuaLoader.isLuacBytecode(neteaseBytes)) {
                            extractFromBytecode(neteaseBytes, regions);
                        } else {
                            String text = new String(neteaseBytes, java.nio.charset.StandardCharsets.UTF_8);
                            extractFromText(text, regions);
                        }
                        Log.i(TAG, "Netease atlas added " + (regions.size() - before) + " regions (total: " + regions.size() + ")");
                    }
                }
            } catch (Exception e) {
                Log.w(TAG, "Failed to load UIPackedAtlasNetease: " + e.getMessage());
            }
