...
public class AIChatDialog {
    ...
    private void showConfigDialog() {
        ...
        // 选择预设模型 button (in-layout, replaces setNeutralButton)
        final Button btnPreset = new Button(context);
        btnPreset.setText("选择预设模型");
        btnPreset.setTextColor(0xFFCCCCCC);
        btnPreset.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
        btnPreset.setBackgroundColor(0xFF3A3A4E);
        LinearLayout.LayoutParams presetLp = new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, dp(40));
        presetLp.setMargins(0, dp(12), 0, dp(8));
        layout.addView(btnPreset, presetLp);

        // 保存设置 button (in-layout, replaces setPositiveButton)
        final Button btnSave = new Button(context);
        btnSave.setText("保存设置");
        btnSave.setTextColor(Color.WHITE);
        btnSave.setTextSize(TypedValue.COMPLEX_UNIT_SP, 15);
        btnSave.setTypeface(android.graphics.Typeface.DEFAULT, android.graphics.Typeface.BOLD);
        btnSave.setBackgroundColor(0xFF1A73E8);
        LinearLayout.LayoutParams saveLp = new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, dp(44));
        layout.addView(btnSave, saveLp);

        // Create dialog with only title/view/cancelable — no AlertDialog buttons
        final AlertDialog configDialog = new AlertDialog.Builder(context)
                .setTitle("AI API 设置")
                .setView(layout)
                .setCancelable(true)
                .create();
