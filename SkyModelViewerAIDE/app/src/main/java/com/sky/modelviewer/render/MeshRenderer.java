...
public class MeshRenderer implements GLSurfaceView.Renderer {
    ...
    private float[] meshIBMInvForAnimBone = null; // 16 * animBoneCount, column-major (inverse of above)
    ...
    public void setAnimation(com.sky.modelviewer.parsing.AnimPackParser.AnimPack anim) {
        ...
        for (int i = 0; i < animBoneCount; i++) {
            ...
            if (rm != null && rm.length >= 16) {
                // Copy as-is: already in correct column-major column-vector format
    ...
    private void createSkinProgram() {
        String vs =
            ...
            "    vec4 skinnedPos = boneMat * vec4(aPosition, 1.0);\n" +
            "    vec4 skinnedNorm = boneMat * vec4(aNormal, 0.0);\n" +
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        for (MeshInstance inst : meshInstances) {
            ...
            // Use skinning shader if mesh has bone weights and animation is loaded
            if (inst.hasBoneWeights && currentAnim != null && skinProgram != 0) {
    ...
    private void renderSkinnedInstance(MeshInstance inst, float[] mvp, float[] mv, float[] normalMatrix) {
        ...
        GLES30.glUniform1i(skinHasTextureLoc, inst.hasTexture ? 1 : 0);
        ...
        // The shader indexes uBoneMatrices by mesh bone index, but boneMatrices is in animpack order.
        // Build a remapped array: for each mesh bone index, use its corresponding animpack bone's matrix.
        if (boneMatrices != null && skinBoneMatricesLoc >= 0) {
            float[] remapped;
            if (meshBoneToAnimBone != null && meshBoneToAnimBone.length > 0) {
                // Build array in mesh bone order
                remapped = new float[meshBoneCount * 16];
                for (int mi = 0; mi < meshBoneCount; mi++) {
                    int ai = meshBoneToAnimBone[mi];
