...
public class SkyResourceResolver {
    ...
    private static final HashMap<String, JSONArray> neteaseOutfitDefsCache = new HashMap<String, JSONArray>();

    private SkyResourceResolver() {}

    /**
     * Public method to get OutfitDefs.json as a JSONArray.
     * Returns null if not found or invalid.
     */
    public static JSONArray getOutfitDefs(String apkPath) {
        return findAndLoadOutfitDefs(apkPath);
    }

    /**
     * 获取网易版OutfitDefs_Netease.json，如果存在的话。
     * 国际版APK没有此文件，返回null。
     */
    public static JSONArray getNeteaseOutfitDefs(String apkPath) {
        return findAndLoadNeteaseOutfitDefs(apkPath);
    }
    ...
    private static HashSet<String> loadShaderNames(String apkPath) {
        ...
        String shaderText = LuaLoader.loadLuaScript(apkPath, "ShaderDefs.lua");
        if (shaderText != null) {
            // DSL format: resource "Shader" "ShaderName"
            Pattern dslPattern = Pattern.compile("resource\\s+\"Shader\"\\s+\"([^\"]+)\"");
            Matcher matcher = dslPattern.matcher(shaderText);
            while (matcher.find()) {
                result.add(matcher.group(1));
            }
            // Curried format: resource("Shader")("ShaderName")
            Pattern curriedPattern = Pattern.compile("resource\\s*\\(\\s*\"Shader\"\\s*\\)\\s*\\(\\s*\"([^\"]+)\"");
            matcher = curriedPattern.matcher(shaderText);
            while (matcher.find()) {
                result.add(matcher.group(1));
            }
            // Function-call format: resource("Shader", "ShaderName", ...)
            Pattern funcPattern = Pattern.compile("resource\\s*\\(\\s*\"Shader\"\\s*,\\s*\"([^\"]+)\"");
            matcher = funcPattern.matcher(shaderText);
            while (matcher.find()) {
                result.add(matcher.group(1));
            }
        }
    ...
    private static JSONArray findAndLoadNeteaseOutfitDefs(String apkPath) {
        if (neteaseOutfitDefsCache.containsKey(apkPath)) return neteaseOutfitDefsCache.get(apkPath);
        String entryPath = SkyAssetScanner.findResourceEntry(apkPath, "OutfitDefs_Netease.json");
        if (entryPath == null) {
            neteaseOutfitDefsCache.put(apkPath, null);
            return null;
        }
        byte[] bytes = SkyAssetScanner.readApkEntry(apkPath, entryPath);
        if (bytes == null) {
        ...
        try {
            JSONArray arr = new JSONArray(json);
            neteaseOutfitDefsCache.put(apkPath, arr);
            return arr;
        } catch (org.json.JSONException e) {
            neteaseOutfitDefsCache.put(apkPath, null);
            return null;
     ...
     * 清除网易衣柜定义缓存（APK切换时调用）。
     */
    public static void clearNeteaseCache() {
        neteaseOutfitDefsCache.clear();
    }
    ...
    private static JSONObject findAndLoadImageDefs(String apkPath) {
        ...
        String text = LuaLoader.loadLuaScript(apkPath, "ImageDefs.lua");
        if (text == null) {
        ...
        // DSL format: resourcedef "Image" "ImageName" { source = "..." }
        Pattern dslPattern = Pattern.compile(
        ...
        // Curried format: resourcedef("Image")("ImageName")({ source = "..." })
        Pattern curriedPattern = Pattern.compile(
            "resourcedef\\s*\\(\\s*\"Image\"\\s*\\)\\s*\\(\\s*\"([^\"]+)\"\\s*\\)\\s*\\(\\s*\\{[^}]*source\\s*=\\s*\"([^\"]+)\"",
            Pattern.DOTALL);
        matcher = curriedPattern.matcher(text);
        while (matcher.find()) {
            try {
                result.put(matcher.group(1), matcher.group(2));
            } catch (org.json.JSONException e) {
        ...
        // Function-call format: resourcedef("Image", "ImageName", { source = "..." })
        Pattern funcPattern = Pattern.compile(
            "resourcedef\\s*\\(\\s*\"Image\"\\s*,\\s*\"([^\"]+)\"\\s*,\\s*\\{[^}]*source\\s*=\\s*\"([^\"]+)\"",
            Pattern.DOTALL);
        matcher = funcPattern.matcher(text);
        while (matcher.find()) {
            try {
                result.put(matcher.group(1), matcher.group(2));
            } catch (org.json.JSONException e) {
