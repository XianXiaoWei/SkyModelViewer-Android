...
public class MeshListAdapter extends BaseAdapter {
    ...
    /**
     * Filter for wardrobe mode: show only mesh entries matching the category keyword,
     * then optionally filtered by a search query. Excludes non-mesh entries.
     */
    public void filterWardrobe(String category, String query) {
        visibleEntries = new ArrayList<MeshCatalogEntry>();
        // Use category + underscore for precise matching (e.g. "body_" not just "body")
        String cat = category != null ? (category + "_").toLowerCase() : "";
        String q = (query != null && !query.trim().isEmpty()) ? query.toLowerCase() : null;
        for (MeshCatalogEntry entry : allEntries) {
            // Show mesh entries matching category keyword
            if ("mesh".equals(entry.fileType)) {
                if (!entry.name.toLowerCase().contains(cat)) continue;
            } else if ("animpack".equals(entry.fileType)) {
                // Always show animpack entries in wardrobe mode for animation selection
            } else {
                continue; // Skip ktx/level
            }
            // If search query provided, must also match
            if (q != null) {
                if (!entry.name.toLowerCase().contains(q) &&
                    !entry.relativePath.toLowerCase().contains(q)) continue;
            }
            visibleEntries.add(entry);
        }
        sortEntries();
