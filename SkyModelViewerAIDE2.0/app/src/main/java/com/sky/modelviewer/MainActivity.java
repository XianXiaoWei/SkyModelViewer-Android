...
public class MainActivity extends Activity {
    ...
    private Button btnModeVisual, btnModeWardrobe;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            // Disable wardrobe button until APK scan completes
                ...
                try {
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            // Enable wardrobe button now that scan is complete
    ...
    private void switchMode(boolean toWardrobe) {
        if (wardrobeMode == toWardrobe) return;
        ...
        if (toWardrobe) {
            ...
            // clearMeshInstances already clears ALL instances (including wardrobe ones)
            // and deletes their GPU buffers, so don't call clearWardrobeMeshes after it
            meshViewport.queueEvent(new Runnable() {
                @Override
                public void run() {
            ...
            // Stop continuous rendering from visualization mode
            meshViewport.setContinuousRender(false);
            ...
            // Stop continuous rendering from animation playback
            meshViewport.setContinuousRender(false);
            animNameOverlay.setText("");
            // Stop animation seek bar updater
            if (animSeekBarUpdater != null) {
            ...
            // Clear all 3D state from wardrobe mode — full isolation
            // clearMeshInstances clears ALL instances and deletes GPU buffers
            meshViewport.queueEvent(new Runnable() {
                @Override
                public void run() {
                    meshViewport.getMeshRenderer().clearAnimation();
