...
public class WardrobeGridAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (isClearItem) {
            ...
            // Load icon — use cached bitmap or placeholder, queue async load
            String iconName = iconNames.get(entry.name);
            if (iconName != null && !iconName.isEmpty()) {
                Bitmap cached = iconBitmaps.get(iconName);
                if (cached != null) {
                    ...
                    synchronized (loadingIcons) {
                        if (!loadingIcons.contains(iconName) && iconLoader != null) {
                            loadingIcons.add(iconName);
                            final String iconKey = iconName;
                            iconExecutor.execute(new Runnable() {
                                @Override
                                public void run() {
                                    Bitmap bmp = null;
                                    try {
                                        bmp = iconLoader.loadIcon(iconKey);
                                    } catch (Exception e) {
                                        // ignore
                                    }
                                    if (bmp != null) {
                                        iconBitmaps.put(iconKey, bmp);
                                    }
                                    synchronized (loadingIcons) {
                                        loadingIcons.remove(iconKey);
                                    }
